...
class AdministratorStatus(IntEnum):
    # 状态 0 正常 1 禁用
    NORMAL = 0
    DISABLE = 1


class AdministratorTokenInDB(DBBaseModel):
    ...
    admin_id: Mapped[int] = mapped_column(
        ...
        nullable=False,
        primary_key=True,
        comment="administrator token id",
    )
    value: Mapped[str] = mapped_column(String(32), nullable=False, comment="token")
    ...
    def __init__(self, admin_id: int, value: str) -> None:
        self.admin_id = admin_id
        self.value = value

    def __repr__(self):
        return f"<AdministratorToken(user_id='{self.admin_id}', token='{self.value}')>"


class AdministratorInDB(DBBaseModel):
    __tablename__ = "administrator"

    id: Mapped[int] = mapped_column(
        Integer,
        Sequence(start=1, increment=1, name="db_administrator_id_sep"),
        primary_key=True,
    ...
    token: Mapped[Optional[AdministratorTokenInDB]] = relationship(
        "AdministratorTokenInDB", backref="admin", uselist=False
    )
    ...
    def find_by_token(
        ...
        stmt = (
            select(cls)
            .join(AdministratorTokenInDB, cls.id == AdministratorTokenInDB.admin_id)
            .options(joinedload(cls.token))
            .where(AdministratorTokenInDB.value == token)
        )
        result = session.scalar(stmt)
        return result
