...
if TYPE_CHECKING:
    ...
    from libs.models import (
        AccountInDB, CompanyInDB, DepartmentInDB,
        PositionInDB, EmployeeInDB, AccountCompanyInDB
    )
...
def command():
    ...
    try:
        ...
        from services.candidate import CandidateService, CandidateCreate, CandidateStatus
        from libs.models import (
            AccountInDB, CompanyInDB, DepartmentInDB, 
            PositionInDB, EmployeeInDB, AccountCompanyRole
        )
...
def create_sample_companies(session: Session, admin_account: 'AccountInDB') -> list['CompanyInDB']:
    """创建示例公司"""
    from libs.models import CompanyInDB, AccountCompanyInDB, AccountCompanyRole
    
    companies = []
    company_names = ["未来科技有限公司", "环球餐饮集团", "智慧教育联盟"]
    
    for name in company_names:
        # 检查公司是否已存在
        existing = session.query(CompanyInDB).filter_by(name=name).first()
        if existing:
            click.echo(f"公司 '{name}' 已存在，跳过创建")
        ...
        company = CompanyInDB(
            name=name,
            description=f"{name}是一家专注于行业发展的创新企业",
        ...
        click.echo(f"创建公司成功: id={company.id}, name={company.name}")
    
    return companies


def create_departments_and_positions(session: Session, admin_account: 'AccountInDB', company: 'CompanyInDB'):
    """为公司创建部门和职位"""
    ...
    for dept_idx, position_name in positions_info:
        if dept_idx < len(departments):
            ...
            click.echo(f"创建职位成功: 公司={company.name}, 部门={departments[dept_idx].name}, 职位={position.name}")
...
def create_employees(session: Session, company: 'CompanyInDB'):
    """为公司创建员工"""
    from libs.models import EmployeeInDB, DepartmentInDB, PositionInDB, EmployeePositionInDB
    ...
    # 名字列表
    names = ["伟", "芳", "娜", "秀英", "敏", "静", "强", "磊", "洋", "艳", "勇", "杰", "娟", "涛", "明", "超"]
    ...
    for i in range(num_employees):
        ...
        if departments and positions:
            ...
            emp_position = EmployeePositionInDB(
                ...
                start_date=datetime.now().date() - timedelta(days=random.randint(30, 365*3)),  # 1个月到3年前入职
                created_at=datetime.now(),
                updated_at=datetime.now()
            )
            session.add(emp_position)
            
            click.echo(f"创建员工成功: 公司={company.name}, 员工={employee.name}, 部门={dept.name}, 职位={position.name}")
        else:
...
def create_candidates(session: Session, admin_account: 'AccountInDB', company: 'CompanyInDB'):
    """为公司创建候选人"""
    ...
    for i in range(num_candidates):
        ...
        click.echo(f"创建候选人成功: 公司={company.name}, 候选人={candidate.name}, 部门={dept.name}, 职位={position.name}, 状态={status}")
