...
def test_create_company_success(
    client: FlaskClient, test_user: AccountInDB, test_session: Session
):
    """测试成功创建公司场景"""
    # 准备创建公司的数据
    ...
    # 验证数据库中的公司记录
    company = (
        test_session.query(CompanyInDB).filter_by(name=company_data["name"]).first()
    )
...
def test_create_company_invalid_request(client: FlaskClient, test_user: AccountInDB):
    """测试无效的请求数据"""
    # 缺少必需的name字段
    invalid_data = {"description": "A test company"}
    response = client.post(
        "/api/companies", data=json.dumps(invalid_data), content_type="application/json"
    )
...
def test_create_company_unauthorized(client: FlaskClient):
    ...
    company_data = {"name": "Test Company", "description": "A test company"}
    response = client.post(
        "/api/companies", data=json.dumps(company_data), content_type="application/json"
    )
