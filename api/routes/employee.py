...
def insert_employee() -> Response:
    ...
    try:
        ...
        # 创建员工服务实例
        service = EmployeeService(session=db.session, account=user)  # type: ignore
...
def get_employee_positions(employee_id: int) -> Response:
    ...
    try:
        ...
        return make_api_response(
            ResponseSchema[List[EmployeePositionSchema]].from_error(
                message=str(e), status=403
            ),
            403,
        )
