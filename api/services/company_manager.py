...
class CompanyService:
    ...
    def validate_member(self, member_id: int) -> Optional[AccountInDB]:
        ...
        stmt = select(AccountInDB).where(
            AccountInDB.id == member_id, AccountInDB.is_active.is_(True)
        )
    ...
    def create_company(
    ...
    ) -> Tuple[Optional[CompanyInDB], Optional[str]]:
        """创建新公司
        ...
        try:
            ...
            new_account_company = AccountCompanyInDB(
                account_id=self.current_user.id, company_id=new_company.id, role="owner"
            )
    ...
    def get_company_by_name(self, name: str) -> Optional[CompanyInDB]:
        ...
        if result:
            ...
            # 检查当前用户是否为公司创建者
            if (
                account_company.account_id == self.current_user.id
                and account_company.role == "owner"
            ):
                return company
        return None
