...
use lib_entity::entities::{
    ...
    employee_position::{self, ActiveModel as EmployeePositionActive, Entity as EmployeePosition, Model as EmployeePositionModel},
    company, department, position,
};
use lib_schema::{PageParams, PageResult};
use lib_schema::models::employee::{InsertEmployee, Gender};
...
impl EmployeeService {
    ...
    pub async fn insert(&self, params: InsertEmployee) -> Result<EmployeeModel, DbErr> {
        match params.id {
            // 更新现有员工
            Some(id) => {
                let employee = if let Some(employee) = self.find_by_id(id.as_u128() as i32).await? {
                    employee
                } else {
                ...
                employee.extra_schema_id = Set(params.extra_schema_id.map(|id| id.as_u128() as i32));
    ...
    /// 
    /// # 返回
    /// - `Result<Option<EmployeeModel>, DbErr>`: 成功返回可能存在的员工模型，失败返回数据库错误
    ...
    pub async fn add_position(&self, params: InsertEmployeePosition) -> Result<EmployeePositionModel, DbErr> {
        // 验证员工是否存在
        let employee_id = params.employee_id.as_u128() as i32;
        if !Employee::find_by_id(employee_id).one(&self.db).await?.is_some() {
            return Err(DbErr::Custom("Employee not found".to_owned()));
        }

        // 验证公司是否存在
        let company_id = params.company_id.as_u128() as i32;
        if !company::Entity::find_by_id(company_id).one(&self.db).await?.is_some() {
            return Err(DbErr::Custom("Company not found".to_owned()));
        }

        // 验证部门是否存在且属于该公司
        let department_id = params.department_id.as_u128() as i32;
        if let Some(dept) = department::Entity::find_by_id(department_id).one(&self.db).await? {
            if dept.company_id != company_id {
                return Err(DbErr::Custom("Department does not belong to the company".to_owned()));
            }
        } else {
        ...
        // 验证职位是否存在且属于该公司
        let position_id = params.position_id.as_u128() as i32;
        if let Some(pos) = position::Entity::find_by_id(position_id).one(&self.db).await? {
            if pos.company_id != company_id {
                return Err(DbErr::Custom("Position does not belong to the company".to_owned()));
            }
        } else {
        ...
        match params.id {
            Some(id) => {
                let relation = if let Some(relation) = EmployeePosition::find_by_id(id.as_u128() as i32).one(&self.db).await? {
                    relation
                } else {
                    return Err(DbErr::Custom("Employee position relation not found".to_owned()));
                };
            ...
            None => {
                ...
                EmployeePosition::insert(relation)
                    .exec_with_returning(&self.db)
                    .await
            }
        }
...
mod tests {
    ...
    async fn test_update_employee() {
        ...
        let employee = service.insert(create_params).await
            .expect("创建测试员工失败");
        
        // 测试成功更新
        let update_params = InsertEmployee {
            id: Some(Uuid::from_u128(employee.id as u128)),
            name: "李四改".to_string(),
        ...
        assert_eq!(updated_employee.email, Some("lisi.new@example.com".to_string()));
    ...
    async fn test_delete_employee() {
        ...
        let employee = service.insert(params).await
            .expect("创建测试员工失败");
        ...
        // 验证删除后无法找到
        let find_result = service.find_by_id(employee.id).await
            .expect("查询删除的员工失败");
