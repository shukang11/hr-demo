...
impl DepartmentService {
    ...
    pub async fn insert(&self, params: InsertDepartment) -> Result<Model, DbErr> {
        match params.id {
            // 更新现有部门
            Some(id) => {
                let department = if let Some(department) = self.find_by_id(id.as_u128() as i32).await? {
                    department
                } else {
                ...
                department.parent_id = Set(params.parent_id.map(|id| id.as_u128() as i32));
                department.company_id = Set(params.company_id.as_u128() as i32);
                department.leader_id = Set(params.leader_id.map(|id| id.as_u128() as i32));
            ...
            None => {
                let department = ActiveModel {
                    ...
                    parent_id: Set(params.parent_id.map(|id| id.as_u128() as i32)),
                    company_id: Set(params.company_id.as_u128() as i32),
                    leader_id: Set(params.leader_id.map(|id| id.as_u128() as i32)),
    ...
    /// 
    /// # 返回
    /// - `Result<Option<Model>, DbErr>`: 成功返回可能存在的部门模型，失败返回数据库错误
...
mod tests {
    ...
    async fn test_update_department() {
        ...
        let department = service.insert(create_params).await
            .expect("创建测试部门失败");
        
        // 测试成功更新
        let update_params = InsertDepartment {
            id: Some(Uuid::from_u128(department.id as u128)),
            name: "新部门名".to_string(),
            parent_id: None,
            company_id: Uuid::from_u128(department.company_id as u128),
            leader_id: None,
            remark: None,
        };
    ...
    async fn test_delete_department() {
        ...
        let department = service.insert(params).await
            .expect("创建测试部门失败");
        ...
        // 验证删除后无法找到
        let find_result = service.find_by_id(department.id).await
            .expect("查询删除的部门失败");
    ...
    async fn test_query_departments() {
        ...
        let result = service.find_by_company(company_id.as_u128() as i32, &page_params).await
            .expect("分页查询失败");
        ...
        let result = service.search_by_name(
            company_id.as_u128() as i32,
            "研发",
            &page_params
        ).await.expect("搜索部门失败");
        assert_eq!(result.total, 1, "搜索结果数量不正确");
        assert!(result.items.iter().any(|d| d.name.contains("研发")), 
            "搜索结果中没有包含关键字的部门");
    }
} 