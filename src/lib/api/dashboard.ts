...
export async function getEmployeeOverview(
  ...
  const response = await serverAPI
    .get(`dashboard/employee-overview/${companyId}`, {
      searchParams: {
        start_time: startTime.toString(),
        end_time: endTime.toString(),
      },
...
export async function getRecruitmentStats(
  ...
  const response = await serverAPI
    .get(`dashboard/recruitment-stats/${companyId}`, {
      searchParams: {
        start_time: startTime.toString(),
        end_time: endTime.toString(),
      },
...
export async function getOrganizationStats(
  ...
  const response = await serverAPI
    .get(`dashboard/organization-stats/${companyId}`, {
      searchParams: {
        start_time: startTime.toString(),
        end_time: endTime.toString(),
      },
...
// 获取子公司聚合数据
export async function getSubsidiariesStats(
  parentId: number,
  ...
  if (subsidiaryIds && subsidiaryIds.length > 0) {
    searchParams.subsidiary_ids = subsidiaryIds.join(",");
  }

  const response = await serverAPI
    .get(`dashboard/subsidiaries-stats/${parentId}`, {
      searchParams,
    })
    .json<ApiResponse<DashboardStats>>();
  if (!response.data) throw new Error("No data returned");
...
// SWR Hook - 子公司聚合数据
export function useSubsidiariesStats(
  parentId: number | undefined,
  subsidiaryIds?: number[]
) {
  return useSWR(
    parentId
      ? ["dashboard", "subsidiaries-stats", parentId, subsidiaryIds?.join(",")]
      : null,
    async () => {
      if (!parentId) return null;
      return await getSubsidiariesStats(parentId, subsidiaryIds);
    }
  );
}
